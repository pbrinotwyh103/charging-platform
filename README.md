...
无需启动服务器或登录，可使用演示模式直接查看进行中的充电任务。页面中的电量、功率、时长、费用和更新时间会每秒刷新：

```bash
cd ~/charging-platform
./build/bin/charging_user_client --demo-charging
```

不带 `--demo-charging` 参数启动时，客户端仍按正常方式连接服务器。

## Linux 中文输入

地址输入框支持中文输入法。程序会保留已有的 `QT_IM_MODULE` 配置；未配置且系统存在 Qt IBus 插件时，会自动使用 IBus。Ubuntu 尚未安装中文输入法时可执行：

```bash
sudo apt install ibus ibus-libpinyin
```

安装后注销并重新登录桌面，在系统输入法设置中添加“中文（智能拼音）”。

在 WSLg 中，程序会自动切换到 X11 后端、启动 direct 模式的 IBus，并尝试激活智能拼音，从而避免候选框在输入每个字母后闪退。若显式设置了 `QT_QPA_PLATFORM`，程序会尊重该设置，不覆盖平台后端。
