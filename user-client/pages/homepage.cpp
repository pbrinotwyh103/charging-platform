...
HomePage::HomePage(QWidget *parent)
    : QWidget(parent)
{
    auto *layout = new QVBoxLayout(this); layout->setContentsMargins(20,20,20,20); layout->setSpacing(10);
    auto *title = new QLabel(QStringLiteral("附近充电站"), this); QFont f=title->font(); f.setPointSize(18); f.setBold(true); title->setFont(f); layout->addWidget(title);
...
void HomePage::setStations(const QJsonArray &stations){m_list->clear();if(stations.isEmpty()){renderEmpty(QStringLiteral("附近暂无可用充电站"));return;}const auto sorted=StationUtils::sortByDistance(stations,m_latitude,m_longitude);for(const auto&v:sorted){const auto o=v.toObject();const double price=o.contains(QStringLiteral("priceCentsPerKwh"))?o.value(QStringLiteral("priceCentsPerKwh")).toDouble()/100.0:o.value(QStringLiteral("price")).toDouble();auto*i=new QListWidgetItem(QStringLiteral("%1\n%2\n电价 ¥%3/度 · 空闲 %4/%5 · %6 km").arg(o.value("name").toString(),o.value("address").toString()).arg(price,0,'f',2).arg(o.value("availablePiles").toInt()).arg(o.value("totalPiles").toInt()).arg(o.value("distanceKm").toDouble(),0,'f',1),m_list);i->setData(Qt::UserRole,QVariant::fromValue(o));}m_status->setText(QStringLiteral("已找到 %1 个充电站，按距离排序").arg(sorted.size()));}
