    ...
    : QWidget(parent)
{
    ...
    auto *row = new QHBoxLayout; m_region=new QComboBox(this); m_region->addItems({QStringLiteral("全部区域"),QStringLiteral("深圳市")}); m_address=new QLineEdit(this); m_address->setPlaceholderText(QStringLiteral("输入地址")); row->addWidget(m_region); row->addWidget(m_address,1); layout->addLayout(row);
