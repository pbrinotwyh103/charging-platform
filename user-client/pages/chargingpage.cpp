...
void ChargingPage::setReservation(const QJsonObject &station, const QJsonObject &pile)
{
    ...
    m_orderLabel->setText(QStringLiteral("%1\n电桩 %2 · %3 kW\n预约编号：R%4")
                              ...
                              .arg(m_pile.value(QStringLiteral("reservationId")).toInteger()));
...
void ChargingPage::renderSnapshot()
{
    ...
    const int seconds = m_snapshot.value(QStringLiteral("durationSec"))
                            .toInt(m_snapshot.value(QStringLiteral("durationSeconds")).toInt());
    const double energyKwh = m_snapshot.contains(QStringLiteral("energyKwh"))
                                 ? m_snapshot.value(QStringLiteral("energyKwh")).toDouble()
                                 : m_snapshot.value(QStringLiteral("energyWh")).toDouble() / 1000.0;
